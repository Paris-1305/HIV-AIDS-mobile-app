...
@Component({
  selector: 'app-safe-sex-practices',
  templateUrl: './safe-sex-practices.page.html',
  styleUrls: ['./safe-sex-practices.page.scss'],
  standalone: true,
...
export class SafeSexPracticesPage implements OnInit,OnDestroy,ViewWillLeave {
 page_id: number | null = null;  // Unique ID for Testing Page
  ...
  onLinkClick() {
    ...
    if (this.page_id && this.user_id) {
      console.log("✅ Calling recordClick() with:", this.page_id, this.user_id, this.time_spent,
         this.userInteractionService.clicks + 1);
